...
const MoviePage = () => {
  ...
  const filteredMovies = movies
    .filter(movie => {
      // only include movies releasing within the next 20 days or already released
            ...
            >
              <div className="w-[150px] h-[280px] rounded-[8px] mb-[-30px] overflow-hidden cursor-pointer text-center flex flex-col justify-start transition-transform transition-shadow duration-300 ease hover:-translate-y-[5px] 
                ...
                <img
                  src={movie.image}
                  alt={movie.movieName}
                  className="w-full h-[200px] object-cover 2xl:w-full 2xl:h-[320px]"
                />
                <h3 className="mt-[5px] mb-[10px] text-[1rem] font-bold text-left hover:text-black-900">
                  {movie.movieName}
                </h3>
              </div>
            </Link>
